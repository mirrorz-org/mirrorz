...
(async () => {
  ...
  const snapshotBaseline = artifacts.data.artifacts.find(
    (artifact) => artifact.name == "screenshot-baseline"
  );
  if (snapshotBaseline) {
    ...
    fs.writeFileSync(
      process.argv[2],
      Buffer.from(artifact.data as ArrayBuffer)
    );
  } else {
    console.error(
      "Last successful run on default branch doesn't have snapshot baseline as artifact."
    );
    process.exit(1);
  }
})();
